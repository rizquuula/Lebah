...
impl WorktreePort for WorktreeManager {
    ...
    fn apply_links(
        ...
        if !wt_path.is_dir() {
            log::warn!("[worktree] apply_links: worktree dir does not exist: {:?}", wt_path);
            return Ok(());
        }
        ...
        for link in links {
            ...
            if !source.exists() {
                log::debug!("[worktree] apply_links: source does not exist, skipping: {:?}", source);
                continue;
            }
            ...
            if dest.exists() || dest.symlink_metadata().is_ok() {
                log::debug!("[worktree] apply_links: dest already exists, skipping: {:?}", dest);
                continue;
            }

            #[cfg(unix)]
            std::os::unix::fs::symlink(&source, &dest).map_err(|e| {
                log::error!("[worktree] apply_links: failed to symlink {:?} -> {:?}: {}", dest, source, e);
                ApplicationError::Io(e)
            })?;
            ...
            #[cfg(windows)]
            {
                if source.is_dir() {
                    std::os::windows::fs::symlink_dir(&source, &dest).map_err(|e| {
                        log::error!("[worktree] apply_links: failed to symlink dir {:?} -> {:?}: {}", dest, source, e);
                        ApplicationError::Io(e)
                    })?;
                } else {
                    std::os::windows::fs::symlink_file(&source, &dest).map_err(|e| {
                        log::error!("[worktree] apply_links: failed to symlink file {:?} -> {:?}: {}", dest, source, e);
                        ApplicationError::Io(e)
                    })?;
                }
            }

            log::info!("[worktree] apply_links: symlinked {:?} -> {:?}", dest, source);
        }
...
mod tests {
    ...
    fn apply_links_creates_symlink_for_existing_file() {
        ...
        mgr.apply_links(&project_path, &worktree, &[".env".to_string()]).unwrap();

        let symlink = tmp.path().join(".claude").join("worktrees").join("my-feature").join(".env");
    ...
    fn apply_links_creates_symlink_for_existing_dir() {
        ...
        let mgr = WorktreeManager::new();
        mgr.apply_links(&project_path, &worktree, &["node_modules".to_string()]).unwrap();

        let symlink = tmp.path().join(".claude").join("worktrees").join("my-feature").join("node_modules");
    ...
    fn apply_links_skips_nonexistent_source() {
        ...
        let dest = tmp.path().join(".claude").join("worktrees").join("wt").join("does_not_exist");
        assert!(!dest.exists());
    }
    ...
    fn apply_links_skips_empty_entries() {
        ...
        let mgr = WorktreeManager::new();
        let result = mgr.apply_links(&project_path, &worktree, &["".to_string(), "  ".to_string()]);
        assert!(result.is_ok());
    }
}
